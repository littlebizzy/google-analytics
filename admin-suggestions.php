 ...
 */
final class GASLBZ_Admin_Suggestions {
	...
	private $required = array(
		'sc-littlebizzy' => array(
			'name' => 'StatCounter',
			'desc' => 'Inserts StatCounter tracking code just above the closing body tag to ensure the fastest loading speed and to avoid conflicting with any other scripts.',
			'filename' => 'statcounter.php',
		),
	...
	public static function instance() {
		...
		if (!isset(self::$instance))
			self::$instance = new GASLBZ_Admin_Suggestions;
	...
	*/
	private function __construct() {
		...
		$timestamp = (int) get_option( 'gaslbz_dismissed_on' );

		if ( empty( $timestamp ) || ( time() - $timestamp ) > (90 * 86400) ) {

			// Check AJAX submit
			if (defined('DOING_AJAX') && DOING_AJAX) {
				add_action( 'wp_ajax_gaslbz_dismiss', array(&$this, 'dismiss'));
...
<script type="text/javascript">jQuery(function($) {	$(document).on('click', '.gaslbz-dismiss .notice-dismiss', function() { $.post(ajaxurl, {'action':'gaslbz_dismiss','nonce':$(this).parent().attr('data-nonce')}); }); });</script>
	<?php }
	...
	public function dismiss() {
		if (!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'gaslbz-dismiss')) {
			update_option('gaslbz_dismissed_on', time(), true);
		}
	}
	...
	public function admin_notices() {
		...
		$plugin_data = get_plugin_data( GASLBZ_FILE );

		?><div class="gaslbz-dismiss notice notice-success is-dismissible" data-nonce="<?php echo wp_create_nonce('gaslbz-dismiss'); ?>">
